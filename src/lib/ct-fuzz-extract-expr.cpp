...
#include "ct-fuzz-extract-expr.h"
...
namespace {
  ...
  Function *getContractExpr(Function *F, CallInst *I) {
    ...
    Function *NewF = Function::Create(FT, F->getLinkage(), "$expr", F->getParent());
...
bool ExtractExprs::runOnModule(Module &M) {
  bool modified = false;
...
void ExtractExprs::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.addRequired<LoopInfoWrapperPass>();
  AU.addRequired<DominatorTreeWrapperPass>();
}

// Pass ID variable
char ExtractExprs::ID = 0;

// Register the pass
static RegisterPass<ExtractExprs>
X("extract-exprs", "Extract Contracts");
